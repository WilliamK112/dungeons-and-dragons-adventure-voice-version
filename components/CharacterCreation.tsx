...
import FaceCaptureOption from './FaceCaptureOption';
...
const CharacterCreation: React.FC<CharacterCreationProps> = ({ onSubmit, isLoading, error }) => {
  ...
  const handleFaceImageChange = useCallback((index: number, dataUrl: string | null) => {
    setPlayers(current => current.map((p, i) => i === index ? { ...p, faceImageUrl: dataUrl } : p));
  }, []);

  const handlePlayerChange = (index: number, field: keyof Omit<PlayerData, 'portraitUrl' | 'faceImageUrl' | 'isGeneratingPortrait'>, value: string) => {
  ...
  const handleGeneratePortrait = useCallback(async (index: number) => {
    ...
    try {
      const imageUrl = player.faceImageUrl
        ? await geminiService.generateFaceSwapPortrait(player.faceImageUrl, player.portraitPrompt)
        : await geminiService.generateCharacterPortrait(player.portraitPrompt);
      ...
      <div className="max-w-5xl mx-auto bg-gray-900/50 backdrop-blur-sm p-8 rounded-lg border border-amber-800/20 shadow-lg shadow-amber-800/5 animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div key={index} className="bg-gray-800/40 p-5 rounded-lg border border-amber-800/20 flex flex-col gap-4">
                      ...
                      <div className="mt-2 p-4 bg-gray-900/50 rounded-lg border border-amber-800/20">
                          ...
                          <FaceCaptureOption
                            ...
                            onFaceImageChange={(url) => handleFaceImageChange(index, url)}
                          ...
                          <div className='flex gap-4 items-start mt-4'>
                              <div className='flex-grow space-y-2'>
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    >
                                        {player.isGeneratingPortrait ? <LoadingSpinner size="sm" /> : (player.portraitUrl ? 'Regenerate' : (player.faceImageUrl ? 'Generate Face-Swap Portrait' : 'Generate Portrait'))}
                                    </button>
                                    <button
                                        type="button"
          ...
          <div className="text-center pt-4">
            ...
            >
              {isLoading ? <LoadingSpinner size="sm" /> : 'Begin Adventure'}
            </button>
          </div>
        </form>
      </div>
