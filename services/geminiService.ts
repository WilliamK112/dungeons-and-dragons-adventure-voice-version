...
/**
 * Generate a character portrait using a face reference image (face-swap style).
 * Preserves the person's facial identity while applying the D&D character style.
 */
export const generateFaceSwapPortrait = async (faceImageDataUrl: string, prompt: string): Promise<string> => {
    try {
        const ai = getAI();
        const mimeMatch = faceImageDataUrl.match(/^data:(image\/[a-z]+);base64,/);
        const mimeType = mimeMatch?.[1] || 'image/jpeg';
        const base64Data = faceImageDataUrl.split(',')[1];
        if (!base64Data) {
            throw new Error("Invalid face image data URL.");
        }

        const fullPrompt = `IDENTITY ANCHOR: The person in this photo is the absolute source of truth for facial identity.

TASK: Create a waist-up fantasy character portrait based on the style: "${prompt}". Dungeons and dragons character art style, digital painting, neutral background.

STRICT CONSTRAINTS:
1. FACIAL FIDELITY: Replicate the EXACT eyes, nose, mouth, face shape, and unique facial landmarks of the person in the reference image. The likeness must be 1:1.
...
4. QUALITY: Ensure high detail, sharp focus on the eyes, and professional cinematic composition.`;
        ...
        console.error("Error calling Gemini API for face-swap portrait:", e);
        const friendlyErrorMessage = parseGeminiError(e);
        throw new Error(`Failed to generate face-swap portrait. ${friendlyErrorMessage}`);
    }
};
